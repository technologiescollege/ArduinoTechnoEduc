...
extern "C"
{
// EDU : Branchement de la broche et définition de la vitesse //////////////////////////////////////////////////////
void brancherEmetteur(uint8_t pin, uint16_t speed){
	vw_set_tx_pin(pin); 		// Config pin
	vw_setup(speed); 			// Config vitesse par défaut
}
// EDU : Initialisation du récepteur
void brancherRecepteur(int pin, long speed){
	vw_set_rx_pin(pin);    
	vw_setup(speed);
	vw_rx_start();
}
// EDU : Permet de transformer une chaîne de caractères en pointeur pour la fonction vw_send
void emettreTexte(String preMsg){
   char *msg = &preMsg[0];
   vw_send((uint8_t *)msg, preMsg.length());
   return;

}
// EDU : emission complète d'un nombre (int)
void emettreNombre(int preNbrInt){
   int *nbr = &preNbrInt;
   vw_send((uint8_t *)nbr, 4);
   return;

}
// EDU : Réception complète d'un TEXTE
String recevoirTexte(){
...
// EDU : Réception complète d'un nombre (int)
int recevoirNombre(){
      uint8_t buf[VW_MAX_MESSAGE_LEN];
      uint8_t buflen = VW_MAX_MESSAGE_LEN;
	  int nombre = 0;
