...
//TRES IMPORTANT !!!!
//LIRE LE FICHIER \arduino\sketch\4.TechnoZone51\shell_uno28\ATTENTION.odt
...
//******************************************************************************************************
//*  L'interpréteur de commande pour la carte Arduino uno                                              *
//*  Auteur : www.technozone51.fr ( Grosse Christophe )                                                *
//*  Licence : LGPL                                                                                    *
//*  Version 2.8                               du 08/12/14                                             *
//*  Pour Compilation avec Arduino 1.0.5                                                                *
//* ATTENTION : Ne pas oublier de copier les répertoires DS1307, IRremote, LiquidCrystal_I2C contenus  *
//*             dans le ZIP dans le sous-répertoire /libraries de votre logiciel Arduino avant de      *
//*             compiler ce programme                                                                  *
//******************************************************************************************************

//#define __mega_ver 1          //Commenter cette ligne pour compilation sur UNO, DECIMILLIA, FREEDUINO ...
//*******************************************************************************************************
//*   IMPORTANT : La capacité mémoire de l'Arduino UNO étant beaucoup plus petite que la MEGA,          *
//*               il est impossible d'uploader l'intégralité des fonctionnalités de l'interpréteur      *
//*               de la carte Arduino MEGA. L'utilisateur devra donc faire un choix des fonctionnalités *
//*               qu'il souhaite activer ou non dans ça carte Arduino UNO. Pour faire cela, il suffit   *
//*               de décommenter les lignes ci-dessous pour activer la fonction correspondante ou au    *
//*               contraire de commenter les lignes ci-dessous pour désactiver une fonction et ainsi    *
//*               gagner la place nécessaire pour que ce programme tiennent dans la mémoire de la carte *
//*               Arduino UNO.                                                                          *
//*******************************************************************************************************
#define Activate_Numeric_Output              // ( env 362 Octets )
#define Activate_Numeric_Input               // ( env 192 Octets )
#define Activate_Analogic_Input              // ( env 434 Octets )
#define Activate_Pwm                         // ( env 564 Octets )
#define Activate_Lcd                         // ( env 7 068 Octets )
//#define Activate_Ir_Remote                   // ( env 6 596 Octets )
#define Activate_Generic_I2c                 // ( env 1 376 Octets )
//#define Activate_Compiler                    // ( env 7 660 Octets )
//#define Activate_Real_Time_Clock             // ( env 4 454 Octets )
#define Activate_Servo_Motor                 // ( env 1 546 Octets )
#define Activate_Buzzer                      // ( env 928 Octets )
...
#define first_pin_eeprom 0    //Adresse de départ de la table de brochage dans l'EEPROM
#define last_pin_eeprom 19    //Adresse de fin de la table de brochage dans l'EEPROM
...
#define baud_rate 132         //Adresse en EEPROM de la vitesse de communication série

#define first_opcode 140     //Adresse de départ du slot mémoire interne EEPROM de l'arduino
...
void setup()   { 
  ...
  for (int i=first_pin_eeprom;i<=last_pin_eeprom;i++) {
   switch (EEPROM.read(i)) {
     //Serial.print("Broche n°");
     //Serial.write(i);
     #if defined(Activate_Numeric_Output)
     case 1 : //C'est une sortie numérique
              pinMode(i,OUTPUT);
              digitalWrite(i,LOW);
              //Serial.println(" is a numeric output");
              break;
     #endif
     #if defined(Activate_Numeric_Input)    
     case 2 : //C'est une Entrée numérique
              pinMode(i,INPUT);
              //Serial.println(" is a numeric input");
              break;
     #endif
     #if defined(Activate_Analogic_Input)
     case 3 : //C'est une Entrée analogique
              //Serial.println(" is a analogic input");
              break;
     #endif
     #if defined(Activate_Pwm)     
     case 4 : //C'est une sortie PWM
              ...
              //Serial.println(" is a PWM output");
              break;
     #endif
     #if defined(Activate_Servo_Motor)     
     case 5 : //C'est une sortie servo-moteur
              ...
              #endif
              //Serial.println(" is a Servo Motor output");
              break;  
     #endif
     #if defined(Activate_Ir_Remote)         
