...
/*EDU US*/	void RFID125::branch(int8_t pin1, int8_t pin2){
	...
	//---- Gestion codes
		...
		temp=2000;												// Duree de vie du code lu
...
/*EDU US*/	void RFID125::writeCode(String code){
	...
	//---- On vérifie que le code n'est pas déjà présent
		if(codesStringRam.indexOf(code)>0){
			Serial.println("CODE DEJA PRESENT");
			return;
		}
	...
	//---- On enregistre le code RFID et le séparateur
		codesStringRam+=code;
		codesStringRam+=' ';
		//DEBUG2("EN MEMOIRE: ", codesStringRam); 	
	
	//---- On gère le Serial-infos
		if(m_serialInfos==true){
			Serial.print("LISTE\t");
			Serial.println(codesStringRam);
		}
...
/*EDU US*/	void RFID125::clearCode(String code){
	code = code+' ';
		codesStringRam.replace(code, "");
}
...
/*EDU US*/	bool RFID125::codeIsPresent(String code, bool lowLevel){
	...
	//---- On crée une chaîne avec un espace pour garantir la prise en compte du code complet
		...
		//DEBUGLN("FIN");
		//DEBUG("Base : DEBUT");
		//DEBUG(codesStringRam);
		//DEBUGLN("FIN");
	//---- On  calcule la position du code (chaîne) dans la base (-1 = chaîne non trouvée) 
	
		posSubStringCode=codesStringRam.indexOf(subStringCode);
...
/*EDU US*/ void RFID125::setTempo(int32_t tempo){
		...
		temp=tempo;
		if(m_serialInfos==true){Serial.print("TEMPO\t");Serial.print(temp);Serial.println(" ms\n");}
}
...
/*EDU US*/	void RFID125::checkRamBase(){
	...
	//RAPPEL : 
	//		> codesStringRam est la chaîne qui contient tous les codes en RAM séparés par un espace
	//		> RAMlist, RAMliste sont des alias utilisateur
	
	//---- On mesure la longueur de la RAMlist
		int16_t lenCode=codesStringRam.length();
	//---- on prélève le dernier caractère de la chaîne
		char lastCharCode=codesStringRam[lenCode-1];
