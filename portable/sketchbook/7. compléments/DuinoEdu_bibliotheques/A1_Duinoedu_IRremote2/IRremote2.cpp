...
//=== GESTION ECHANGES DONNEES ENTRE 2 CARTES ARDUINO
  ...
  /*EDU US*/ void IRsend2::sendNumber(int32_t _number, uint16_t _interval){
...
void IRsend2::enableIROut(int khz) {
...
IRrecv2::IRrecv2(int recvpin){
  irparams.recvpin = recvpin;
  irparams.blinkflag = 0;
}

// initialization
void IRrecv2::enableIRIn() {
  cli();
...
void IRrecv2::blink13(int blinkflag){
...
//==== GESTION TELECOMMANDE (NEC)
// EDU : lit le code et retourne le code (long) 

// ---------- Branchement et initialisation du module
/*FR*/ void IRrecv2::brancher(){ branch();}
/*US*/ void IRrecv2::branch(){
	enableIRIn();
	m_lifeLongPush=200;				// Durée de vie minimale d'un appui long
	m_dateLongPush=0;				// Date du dernier code d'appui long
	m_lastDateLongPush=0;			// Date de l'avant dernier code d'appui long
}

// ---------- Lecture du code et retour type long
/*EDU FR*/ unsigned long IRrecv2::lireCodeIr(int option){ return codeIrReadLong(option); }
/*EDU US*/ unsigned long IRrecv2::codeIrReadLong(int option){
...
/*EDU FR*/ bool IRrecv2::testerTouche(String touche,int option){return testTouch(touche,option);}
/*EDU US*/ bool IRrecv2::testTouch(String touch,int option){
...
//=== GESTION ECHANGES DONNEES ENTRE 2 CARTES ARDUINO
/*EDU US*/ int32_t IRrecv2::receiveNumber(int16_t _memoryDelay){
	static decode_results 	results;
	static int32_t 			dataReceived 	= 0;
	static uint32_t 		lasteTime 		= 0;
	//static bool   			datReady 		= decode(&results);
	
	//--- Gestion de la nouvelle donnée reçue
		if (decode(&results)) {
			dataReceived = results.value;
			resume(); // Receive the next value
			lasteTime = millis();
			//Serial.println("d:"+dataReceived);
			return dataReceived;
		}
	//--- Gestion de l'absence de donnée reçue
		
		//-- Si on doit mémoriser la donnée en permanence (-1)
			if(_memoryDelay==DUINOEDU_MEMORISER){
				return dataReceived;
			}
		
		//-- Si la donnée ne doit pas être mémorisée (elle a déjà été envoyée 1 fois)
			if(_memoryDelay==0){
				return 0;
			}

		//-- Si on doit la mémoriser pendant un certain temps (2s, 200ms ou un autre temps)
			//- Temps non dépassé
				if( (_memoryDelay>0) && (millis()<lasteTime+_memoryDelay) ){
					return dataReceived;
				}
			//-- Donnée périmée
				else {
...
int IRrecv2::decode(decode_results *results) {
...
long IRrecv2::decodeNEC(decode_results *results) {
  long data = 0;
...
long IRrecv2::decodeSony(decode_results *results) {
  long data = 0;
...
long IRrecv2::decodeSanyo(decode_results *results) {
  long data = 0;
...
// Looks like Sony except for timings, 48 chars of data and time/space different
long IRrecv2::decodeMitsubishi(decode_results *results) {
...
int IRrecv2::getRClevel(decode_results *results, int *offset, int *used, int t1) {
...
long IRrecv2::decodeRC5(decode_results *results) {
...
long IRrecv2::decodeRC6(decode_results *results) {
...
long IRrecv2::decodePanasonic(decode_results *results) {
...
long IRrecv2::decodeJVC(decode_results *results) {
    long data = 0;
    int offset = 1; // Skip first space
    // Check for repeat
...
int IRrecv2::compare(unsigned int oldval, unsigned int newval) {
  if (newval < oldval * .8) {
    return 0;
  } 
 ...
 */
long IRrecv2::decodeHash(decode_results *results) {
...
void IRsend2::sendSharp(unsigned long data, int nbits) {
...
void IRsend2::sendDISH(unsigned long data, int nbits){
  enableIROut(56);
  mark(DISH_HDR_MARK);
