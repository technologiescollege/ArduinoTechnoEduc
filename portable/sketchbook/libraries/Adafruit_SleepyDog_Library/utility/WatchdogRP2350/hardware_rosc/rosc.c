...
#include "include/hardware/rosc.h"
...
void rosc_set_freq(uint32_t code) {
  rosc_write(&rosc_hw->freqa,
             (ROSC_FREQA_PASSWD_VALUE_PASS << ROSC_FREQA_PASSWD_LSB) |
                 (code & 0xffffu));
  rosc_write(&rosc_hw->freqb,
             (ROSC_FREQA_PASSWD_VALUE_PASS << ROSC_FREQA_PASSWD_LSB) |
                 (code >> 16u));
}

void rosc_set_range(uint range) {
  // Range should use enumvals from the headers and thus have the password
  // correct
  rosc_write(&rosc_hw->ctrl,
             (ROSC_CTRL_ENABLE_VALUE_ENABLE << ROSC_CTRL_ENABLE_LSB) | range);
}
