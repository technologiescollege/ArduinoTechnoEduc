...
#ifdef DEBUG_ACTIVE
extern U8 datadebug[40];
extern U8 status_change;
#endif
...
void zg_recv(U8* buf, U16* len)
{
  ...
  *len = ZGSTOHS( ptr->dataLen );
  ...
  memcpy(&zg_buf[0], &(ptr->dstAddr), 6);
  memcpy(&zg_buf[6], &(ptr->srcAddr), 6);
  memcpy(&zg_buf[12],&zg_buf[29], *len);
#ifdef DEBUG_ACTIVE
 copydatadebug();
 datadebug[0]=(ptr->dataLen)&0x00ff;
 datadebug[1]= (ptr->dataLen);
 datadebug[2]=(*len)&0x00ff;
#endif
 *len += 12;
}
...
void copydatadebug(){
	memcpy(datadebug, zg_buf , 40);
}

void copymessage(char * message)
{
	memcpy(datadebug,message,40);
}
...
void zg_drv_process()
{
    ...
    case ZG_MAC_TYPE_MGMT_CONFIRM:
      if (zg_buf[3] == ZG_RESULT_SUCCESS) {
        ...
        case ZG_MAC_SUBTYPE_MGMT_REQ_GET_PARAM:
          ...
          mac[4] = zg_buf[11];
          mac[5] = zg_buf[12];
          zg_drv_state = DRV_STATE_CREATE_PROFILE;
          break;
        case ZG_MAC_SUBTYPE_MGMT_REQ_SET_PARAM:
          zg_drv_state = DRV_STATE_GET_MAC;
          break;
        case ZG_MAC_SUBTYPE_CP_CREATE_PROFILE:
          profileID = 1;//zg_buf[7];
          zg_drv_state = DRV_STATE_SET_SSID;
          break;
        case ZG_MAC_SUBTYPE_CP_SET_ELEMENT_SUBTYPE:
          if ( zg_drv_state == DRV_STATE_IDLE_SSID)
            zg_drv_state = DRV_STATE_SET_NETWORKTYPE;
          else if ( zg_drv_state == DRV_STATE_IDLE_NETWORKTYPE)
            zg_drv_state = DRV_STATE_SETUP_SECURITY;
          else if ( zg_drv_state == DRV_STATE_IDLE_SECURITY)
            zg_drv_state = DRV_STATE_START_CONN;
          break;
        case ZG_MAC_SUBTYPE_CM_CONNECT_SUBYTPE:          
          zg_drv_state = DRV_STATE_CHECK_NETWORK;
          break;
        case ZG_MAC_SUBTYPE_CM_GET_CONNECT_SUBYTPE:
          zg_drv_state=DRV_STATE_IDLE_CHECK;
          if (zg_buf[5] == WF_CONNECTED_ADHOC|| zg_buf[5] == WF_CONNECTED_INFRA)
          { 
  ...
  case DRV_STATE_INIT:
    ...
    zg_buf[2] = ZG_MAC_SUBTYPE_MGMT_REQ_SET_PARAM;
    zg_buf[3] = 0;
    zg_buf[4] = ZG_PARAM_ENABLE_MRF24WB0;
    zg_buf[5] = 1;
    spi_transfer(zg_buf, 6, 1);
  ...
  case DRV_STATE_CREATE_PROFILE:
    zg_buf[0] = ZG_CMD_WT_FIFO_MGMT;
    zg_buf[1] = ZG_MAC_TYPE_MGMT_REQ;
    zg_buf[2] = ZG_MAC_SUBTYPE_CP_CREATE_PROFILE;
    spi_transfer(zg_buf, 3, 1);
  ...
  case DRV_STATE_SET_SSID:
    zg_buf[0] = ZG_CMD_WT_FIFO_MGMT;
    zg_buf[1] = ZG_MAC_TYPE_MGMT_REQ;
    zg_buf[2] = ZG_MAC_SUBTYPE_CP_SET_ELEMENT_SUBTYPE;
    zg_buf[3] = profileID;
    zg_buf[4] = WF_CP_ELEMENT_SSID;
    zg_buf[5] = ssid_len;
    memset(&(zg_buf[6]), 0, 32);
    memcpy_P(&(zg_buf[6]), ssid, ssid_len);
    spi_transfer(zg_buf, 6 + ssid_len , 1);

    zg_buf[0] = ZG_CMD_WT_FIFO_DONE;
    spi_transfer(zg_buf, 1, 1);

    zg_drv_state = DRV_STATE_IDLE_SSID;
    break;

  case DRV_STATE_SET_NETWORKTYPE:
    zg_buf[0] = ZG_CMD_WT_FIFO_MGMT;
    zg_buf[1] = ZG_MAC_TYPE_MGMT_REQ;
    zg_buf[2] = ZG_MAC_SUBTYPE_CP_SET_ELEMENT_SUBTYPE;
    zg_buf[3] = profileID;
    zg_buf[4] = WF_CP_ELEMENT_NETWORK_TYPE;
    zg_buf[5] = 1;
    zg_buf[6] = wireless_mode;
    spi_transfer(zg_buf, 7 , 1);

    zg_buf[0] = ZG_CMD_WT_FIFO_DONE;
    spi_transfer(zg_buf, 1, 1);

    zg_drv_state = DRV_STATE_IDLE_NETWORKTYPE;
    break;
  ...
  case DRV_STATE_SETUP_SECURITY:
    ...
    zg_buf[2] = ZG_MAC_SUBTYPE_CP_SET_ELEMENT_SUBTYPE;
    zg_buf[3] = profileID;
    zg_buf[4] = WF_CP_ELEMENT_SECURITY;
    zg_buf[7] =0;
    switch (security_type) 
    {
    case ZG_SECURITY_TYPE_NONE:
      zg_buf[6]=WF_SECURITY_OPEN;
      zg_buf[5] = 2;
      break;
    case ZG_SECURITY_TYPE_WEP:
      zg_buf[6]=WF_SECURITY_WEP_104;
      zg_buf[5] = 2+13;
      memcpy_P(&(zg_buf[8]), wep_keys[0], 13);
      break;
    case ZG_SECURITY_TYPE_WPA:
      zg_buf[6]=WF_SECURITY_WPA_WITH_PASS_PHRASE;
      zg_buf[5] = 2 +security_passphrase_len;
      memcpy_P(&(zg_buf[8]), security_passphrase, security_passphrase_len);
      break;
    case ZG_SECURITY_TYPE_WPA2:
      zg_buf[5]=WF_SECURITY_WPA2_WITH_PASS_PHRASE;
      zg_buf[5] = 2 +security_passphrase_len;
      memcpy_P(&(zg_buf[8]), security_passphrase, security_passphrase_len);
      break;
    default:
    ...
    spi_transfer(zg_buf, 6+zg_buf[5], 1);
    zg_buf[0] = ZG_CMD_WT_FIFO_DONE;
    spi_transfer(zg_buf, 1, 1);

    zg_drv_state = DRV_STATE_IDLE_SECURITY;
    break;
  ...
  case DRV_STATE_START_CONN:
    ...
    zg_buf[0] = ZG_CMD_WT_FIFO_MGMT;
    zg_buf[1] = ZG_MAC_TYPE_MGMT_REQ;
    zg_buf[2] = ZG_MAC_SUBTYPE_CM_CONNECT_SUBYTPE;
    zg_buf[3] = profileID;
    zg_buf[4] = 0;
    spi_transfer(zg_buf, 5, 1);
    ...
    zg_drv_state = DRV_STATE_IDLE_CONNECT;
    break;

  case DRV_STATE_CHECK_NETWORK:
    zg_buf[0] = ZG_CMD_WT_FIFO_MGMT;
    zg_buf[1] = ZG_MAC_TYPE_MGMT_REQ;
    zg_buf[2] = ZG_MAC_SUBTYPE_CM_GET_CONNECT_SUBYTPE;
