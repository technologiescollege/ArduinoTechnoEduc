/******************************************************************************
  ...
  Filename:		WiSeverMoway.cpp
  Description:	Main library code for the WiServerMoway library
 ...
 ******************************************************************************
  ...
  ---------------------------------------------------------------
   ...
   Mark A. Patel		06/05/2009	Revised to work with stack version 1.1,
	...
	Samuel Merino			12/11/2013	Avoid confict with Arduino WiFi library
...
void Server::init(pageServingFunction function) {
...
#ifdef USE_DIG0_INTR
	attachInterrupt(1, zg_isr, FALLING );
#endif
	...
	while(zg_get_conn_state() != 1) {
		zg_drv_process();
#ifdef DEBUG_ACTIVE
		if (zg_get_drv_state() != 5 && zg_get_drv_state() != 16)
		{
			Serial.print("Estado: ");
			Serial.println(zg_get_drv_state());
			Serial.print("Datos: ");
			for (i=0;i<20;i++)
				Serial.print(datadebug[i],HEX);
			Serial.println("");
			Serial.println("");
		}
#endif
...
#ifdef DEBUG_ACTIVE
	if (zg_get_drv_state() != 5 || status_change == 1)
	{
		status_change=0;
		Serial.print("Estado: ");
		Serial.println(zg_get_drv_state());
		...
		Serial.println("");
		/*Serial.print("Datos: ");
		Serial.print((const char*)datadebug);
		/*for (i=0;i<20;i++)
			Serial.print(datadebug[i],HEX);*/
		Serial.println("");
		Serial.println("");
