...
#define VW_MAX_MESSAGE_LEN 30    // Le message max pour 30 est de 27 (valeur par défaut=30)
...
// EDU : Ajouts émetteur -----------------------------------------------
	// EDU : configuration de l'émetteur
	extern void brancherEmetteur(uint8_t pin, uint16_t speed=2000);
	
	// EDU : émission d'un message TEXTE
	extern void emettreTexte(String preMsg);
	
	// EDU : émission d'un NOMBRE
	extern void emettreNombre(int preNbrInt=0);
    


// EDU : Ajouts récepteur --------------------------------------------------
	// EDU : configuration du récepteur
	extern void brancherRecepteur(int pin, long speed=2000);

	// EDU : réception d'un message complet TEXTE
	extern  String recevoirTexte();	
	
	// EDU : réception complète d1 NOMBRE
	extern int recevoirNombre();
    ...
    extern void vw_setup(uint16_t speed=2000);
